                                ...
                                if (biddingAmount > highestBids.get(listingID)) {
                                    if (!highestBidders.containsKey(listingID) || !itemsPerBuyer.containsKey(bidderName)) {
                                        ...
                                        if (numberOfItems == maxBidCount) {
                                            System.out.println("Max Bid Count reached");
                                            return false;
                                        } else {
...
//post-condition: result = 1 or 2 or 3
    ...
    public synchronized int checkBidStatus(String bidderName, int listingID) {
		...
		Item itemDetails = this.itemsAndIDs.get(listingID);
		if(!itemDetails.biddingOpen()){
			String maxBidder = this.highestBidders.get(itemDetails.listingID());
			
			if(this.itemsUpForBidding.contains(itemDetails)){
				this.itemsUpForBidding.remove(itemDetails);
			}
			int sellerCount = this.itemsPerSeller.get(itemDetails.seller()); 
			this.itemsPerSeller.put(itemDetails.seller(), sellerCount-1);
			
            if (maxBidder != null && maxBidder.equals(bidderName)){ 
            	bid = this.highestBids.get(itemDetails.listingID());
            	Integer bidderCount = this.itemsPerBuyer.get(itemDetails);
            	if(bidderCount!=null && bidderCount>0)
            		this.itemsPerBuyer.put(bidderName, bidderCount-1);
            	revenue = revenue + bid;
            	soldItemsCount++;
            	return 1; 
            }
        ...
        for (int i : highestBids.keySet()) {
            if (!itemsAndIDs.get(i).biddingOpen() && highestBids.containsKey(i)) {
                if (highestBids.get(i) != itemsAndIDs.get(i).lowestBiddingPrice()) {
                    totalHighestBids = totalHighestBids + highestBids.get(i);
                }
            }
